                        ...
                        return (
                            <tr key={item.id} className='hover:bg-gray-50 text-[#333333] font-normal text-xs pb-2 hover:cursor-pointer' onClick={() => router.push(`/dashboard/orders/frame/${item.id}`)} >
                                ...
                                <td className='py-4 px-4 border-b relative'>
                                    ...
                                    {viewMoreBtn === item.id && (
                                        <Link href={`/dashboard/orders/frame/${item.id}`} onMouseLeave={ () => toggleMenu(item.id)}  className="absolute right-14 top-3 mt-1 w-32 border bg-white shadow-lg rounded-lg z-10 text-[#333333]">
